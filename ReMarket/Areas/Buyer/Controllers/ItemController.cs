...
namespace ReMarket.Web.Areas.Buyer.Controllers
{
    ...
    public class ItemController : Controller
    {
        ...
        {
            search = InputSanitizer.CleanQueryParam(search);
            location = InputSanitizer.CleanQueryParam(location);

            if (InputSanitizer.ContainsUnsafeText(condition) || InputSanitizer.ContainsUnsafeText(deliveryOption) || InputSanitizer.ContainsUnsafeText(sortBy))
            {
                condition = null;
                deliveryOption = null;
                sortBy = null;
        ...
        [Authorize]
        public IActionResult AddToCart([Bind(Prefix = "CartInput")] ShoppingCart shoppingCart)
        {
            var claimsIdentity = (ClaimsIdentity)User.Identity!;
            var userId = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier)!.Value;

            var item = _unitOfWork.Item.Get(
                i => i.Id == shoppingCart.ItemId && i.Status == ItemStatus.Available,
                includeProperties: "Category");

            if (item == null)
            {
                TempData["error"] = "This item is not available.";
                return RedirectToAction(nameof(Index));
            }

            if (item.SellerId == userId)
            {
                TempData["error"] = "You cannot add your own listing to the cart.";
                return RedirectToAction(nameof(Detail), new { slug = item.Slug });
            }

            if (shoppingCart.Count < 1 || shoppingCart.Count > item.Quantity)
            {
                TempData["error"] = $"Please enter a quantity between 1 and {item.Quantity}.";
                return RedirectToAction(nameof(Detail), new { slug = item.Slug });
            }
            ...
            var cartFromDb = _unitOfWork.ShoppingCart.Get(
                u => u.ApplicationUserId == userId && u.ItemId == shoppingCart.ItemId,
                tracked: true);

            if (cartFromDb != null)
            {
                var newCount = cartFromDb.Count + shoppingCart.Count;
                if (newCount > item.Quantity)
                {
                    TempData["error"] = $"Only {item.Quantity} available in stock (you already have {cartFromDb.Count} in your cart).";
                    return RedirectToAction(nameof(Detail), new { slug = item.Slug });
                }

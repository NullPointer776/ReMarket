...
namespace ReMarket.Web.Areas.Seller.Controllers
{
    ...
    public class ItemController : Controller
    {
        ...
        public async Task<IActionResult> Create(
            [Bind("Name,Description,Price,Quantity,Condition,DeliveryOption,Location,CategoryId")] Item model,
            IFormFile[]? imageFiles)
        {
            ...
            {
                InputSanitizer.SanitizeItem(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Name,Description,Price,Quantity,Condition,DeliveryOption,Location,CategoryId,ImageUrl,MoreImageUrlsJson")] Item model, IFormFile? coverImageFile, IFormFile[]? additionalImageFiles)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (coverImageFile is { Length: > 0 })
                    await ItemImageUpload.ReplaceCoverAsync(_env, existing, coverImageFile);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult DeleteImage(int id, int imageIndex, string? returnSlug, string? returnTo)
        {
            var item = GetOwnedItem(id);
            if (item == null) return NotFound();
        ...
        private void ClearNavigationModelState()
        {
            ...
            ModelState.Remove(nameof(Item.Status));
            ModelState.Remove(nameof(Item.DatePosted));
            ModelState.Remove(nameof(Item.QrCodeUrl));
            ModelState.Remove(nameof(Item.ImageUrl));
            ModelState.Remove(nameof(Item.MoreImageUrlsJson));
            ModelState.Remove(nameof(Item.RejectionReason));
        }

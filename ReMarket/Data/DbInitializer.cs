...
namespace ReMarket.Web.Data
{
    ...
    public static class DbInitializer
    {
        public static async Task InitializeAsync(IServiceProvider services)
        {
            ...
            {
                // Always guarantee the admin has the Admin role and a known working password.
                if (!await userManager.IsInRoleAsync(admin, SD.Role_Admin))
                    await userManager.AddToRoleAsync(admin, SD.Role_Admin);

                if (!admin.EmailConfirmed || admin.LockoutEnd != null)
                {
                ...
                // Reset the admin password to the seeded value so a known credential always works in development.
